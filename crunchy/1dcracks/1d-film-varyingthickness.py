...
class ThinFilmVaryingThickness(ThinFilm):
    def __init__(
        self, model_parameters={}, eps_0=ufl.Identity(2), thickness: Function = None
    ):
        """
        Initialize the model with an extra `thickness` Function.

        Parameters:
        - V: FunctionSpace for the model.
        - thickness: A dolfinx.fem.Function representing the varying thickness.
        """
        super().__init__(model_parameters, eps_0)  # Initialize the base class
        if thickness is None:
            thickness = 1.0
        self.thickness = thickness  # Store the thickness function

    def elastic_energy_density_strain(self, eps, alpha):
        """
        Compute the elastic energy density from the strain, scaled by thickness.
    ...
    def damage_energy_density(self, state):
        ...
        return self.thickness * base_damage_density
...
def run_computation(parameters, storage=None):
    ...
    thickness = Function(V_alpha, name="Thickness")
    ...
    thickness.interpolate(lambda x: 1.0 / 2.0 + x[0] / Lx)
    ...
    with dolfinx.common.Timer("~Debug"):
        ...
        ax = profile.gca()
        ax.set_ylim(0, 2)
        profile.savefig(f"{prefix}/thickness-profile.png")
    ...
    model = ThinFilmVaryingThickness(
        parameters["model"], thickness=thickness, eps_0=eps_t
    )
    # model = ThinFilmVaryingThickness(parameters["model"], eps_0=eps_t)
