...
def run_computation(parameters, storage):
    nameExp = parameters["geometry"]["geom_type"] + "-nosing"
    geom_params = {
    ...
    top_dofs_u = locate_dofs_geometrical(
        V_u, lambda x: np.isclose(x[1], geom_params["H"] / 2 + geom_params["ext"])
    )
    bottom_dofs_u = locate_dofs_geometrical(
        V_u, lambda x: np.isclose(x[1], -geom_params["H"] / 2 - geom_params["ext"])
    )
    ...
    top_dofs_alpha = locate_dofs_geometrical(
        V_alpha, lambda x: np.isclose(x[1], geom_params["H"] / 2 + geom_params["ext"])
    )
    bottom_dofs_alpha = locate_dofs_geometrical(
        V_alpha, lambda x: np.isclose(x[1], -geom_params["H"] / 2 - geom_params["ext"])
    )
...
if __name__ == "__main__":
    ...
    _storage = f"./output/nosing/{signature[0:6]}"
