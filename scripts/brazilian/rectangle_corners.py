...
from crunchy.mesh import mesh_rect_with_rhomboidal_hole as mesh_with_hole
...
def run_computation(parameters, storage):
    ...
    geom_params = {
        "Lx": 1.0,  # Length of the rectangle along the x-axis
        "Ly": 1.0,  # Length of the rectangle along the y-axis
        "angle": 10.0,  # Angle of the rhomboid in degrees
        "axis": 0.01,  # Axis length of the rhomboid
        "lc": 0.05,  # Mesh element size
        "a": 0.1,  # Half-width of the refined region (-a < x < a)
    }
    with dolfinx.common.Timer(f"~Meshing") as timer:
        gmsh_model, tdim = mesh_with_hole(comm, geom_params)
    ...
    u_lb.interpolate(
        lambda x: np.stack(
            [
                np.full_like(x[0], -np.inf),
                np.full_like(x[1], -geom_params["Lx"] / 2 - x[1]),
            ]
        )
    )
    u_ub.interpolate(
    ...
    for f in [u, alpha_lb, alpha_ub, top_disp, u_lb, u_ub]:
    ...
    def top_boundary(x):
        return np.isclose(x[1], geom_params["Lx"] / 2, atol=1e-2)
    ...
    while True:
        ...
        top_disp.interpolate(
            lambda x: np.stack(
                [np.full_like(x[1], 0.3 * t.value), np.full_like(x[1], -t.value)],
                axis=0,
            )
        )
...
def load_parameters(file_path):
    ...
    parameters["geometry"]["geom_type"] = "rect_corners"
