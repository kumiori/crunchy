...
def run_computation(parameters, storage):
    ...
    with dolfinx.common.Timer(f"~Meshing") as timer:
        gmsh_model, tdim = mesh_matrix_fiber_reinforced(comm, parameters["geometry"])
    ...
    zero_disp = dolfinx.fem.Constant(mesh, np.array([0.0, 0.0], dtype=PETSc.ScalarType))
    ...
    def top_boundary(x):
        return np.isclose(x[1], parameters["geometry"]["L"] / 2, atol=1e-2)

    def bottom_boundary(x):
        return np.isclose(x[1], -parameters["geometry"]["L"] / 2, atol=1e-2)

    def fiber_boundary(x):
        return np.isclose(
            np.sqrt(x[0] ** 2 + x[1] ** 2), parameters["geometry"]["R_inner"], atol=1e-2
        )

    boundaries = [(1, top_boundary), (2, bottom_boundary), (3, fiber_boundary)]
    ...
    fiber_dofs = locate_dofs_topological(
        V_u,
        mesh.topology.dim - 1,
        dolfinx.mesh.locate_entities_boundary(mesh, 1, fiber_boundary),
    )
    bcs_u = [
        # dirichletbc(top_disp, top_dofs, V_u),
        dirichletbc(top_disp, top_dofs),
        dirichletbc(bottom_disp, bottom_dofs, V_u),
        dirichletbc(zero_disp, fiber_dofs, V_u),
    ]
    bcs_alpha = []
